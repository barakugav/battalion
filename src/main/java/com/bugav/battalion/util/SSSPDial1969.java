...
import com.bugav.battalion.util.Graph.Edge;
import com.bugav.battalion.util.Graph.WeightFunction;
import com.bugav.battalion.util.Graph.WeightFunctionInt;
...
public class SSSPDial1969 implements SSSP {
	...
	public <E> SSSP.Result<E> calcDistances(Graph<E> g, WeightFunction<E> w0, int source) {
		...
		if (g.edges().size() <= n - 1) {
			...
			Arrays.sort(edges, 0, g.edges().size(), (e1, e2) -> -Integer.compare(w.weightInt(e1), w.weightInt(e2)));
			for (int i = 0; i <= n - 1; i++)
	...
	public <E> SSSP.Result<E> calcDistances(Graph<E> g, WeightFunctionInt<E> w, int source, int maxDistance) {
		...
		for (int u = source;;) {
			for (Edge<E> e : Iter.iterable(g.edges(u))) {
				int v = e.v();
